...
链接完成后，如果配置中存在名为 `codex` 的目标、其 skills 目录的父目录在执行前已存在，且技能库包含 `codex-subagent-routing`，脚本会自动安装并校验四个子 Agent 角色及全局委派规则。此判断兼容只有桌面 App、没有 `codex` 命令的环境；安装位置以配置中的目标目录为准。

- `agents/` 或角色文件缺失时自动补齐，保留已有有效角色定制。
- 全局 `AGENTS.md` 中相同内容不会重复写入；受管理的规则块会更新，其他内容保留，修改前自动备份。
- 无标记的同名旧规则内容不一致、配置损坏或写入失败时，脚本报错退出；此前完成的技能链接仍保留。
- 存在 `AGENTS.override.md` 时安装器会提示，它可能遮蔽全局规则。
- 自动寻找 Python 3.11+（包括 Codex 自带运行环境），也可用 `AGENT_SKILLS_PYTHON=/path/to/python3` 指定。没有可用版本时明确报错，不自动安装依赖。

安装后新建 Codex 任务加载配置，必要时重启 App。配置校验通过不代表已验证实际角色启动。
