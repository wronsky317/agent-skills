#!/usr/bin/env python3
"""
Retrieve structured text content from PowerPoint presentations.

This module provides functionality to:
- Retrieve all text content from PowerPoint shapes
...
Classes:
    ParagraphInfo: Represents a text paragraph with formatting
    ShapeInfo: Represents a shape with position and text content

Main Functions:
    get_text_shapes_inventory: Retrieve all text from a presentation
    write_inventory: Save retrieved data to JSON

Usage:
    python textExtractor.py input.pptx output.json
"""
...
The output JSON includes:
  ...
  - Issue detection: text overflow and shape overlaps
    ...
    try:
        ...
        inventory = get_text_shapes_inventory(input_path, issues_only=args.issues_only)
...
def get_text_shapes_inventory(
    pptx_path: Path, prs: Optional[Any] = None, issues_only: bool = False
) -> InventoryData:
    """Retrieve text content from all slides in a PowerPoint presentation.
    ...
    Returns a nested dictionary: {slide-N: {shape-N: ShapeInfo}}
    Shapes are sorted by visual position (top-to-bottom, left-to-right).
    The ShapeInfo objects contain the full shape information and can be
...
def write_inventory(inventory: InventoryData, output_path: Path) -> None:
