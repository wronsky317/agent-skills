/**
 * slideConverter - Transform HTML slide designs into pptxgenjs slides with positioned elements
 *
 * USAGE:
 ...
 *
 * FEATURES:
 *   - Transforms HTML to PowerPoint with precise positioning
 *   - Handles text, images, shapes, and bullet lists
 *   - Extracts placeholder elements (class="placeholder") with positions
 *   - Processes CSS gradients, borders, and margins
 *
 * VALIDATION:
...
async function getBodyDimensions(page) {
  ...
  const widthOverflowPt = widthOverflowPx * POINTS_PER_PIXEL;
  const heightOverflowPt = heightOverflowPx * POINTS_PER_PIXEL;
...
function validateDimensions(bodyDimensions, pres) {
  const errors = [];
  const widthInches = bodyDimensions.width / PIXELS_PER_INCH;
  const heightInches = bodyDimensions.height / PIXELS_PER_INCH;

  if (pres.presLayout) {
    const layoutWidth = pres.presLayout.width / EMU_PER_INCH;
    const layoutHeight = pres.presLayout.height / EMU_PER_INCH;
...
function validateTextBoxPosition(slideData, bodyDimensions) {
  const errors = [];
  const slideHeightInches = bodyDimensions.height / PIXELS_PER_INCH;
...
async function extractSlideData(page) {
  return await page.evaluate(() => {
    ...
    const pxToInch = (px) => px / PIXELS_PER_INCH;
    const pxToPoints = (pxStr) => parseFloat(pxStr) * POINTS_PER_PIXEL;
    ...
    const parseBoxShadow = (boxShadow) => {
      ...
      const offset = Math.sqrt(offsetX * offsetX + offsetY * offsetY) * POINTS_PER_PIXEL;
              ...
              elements.push({
                ...
                shape: {
                  ...
                  rectRadius: (() => {
                    ...
                    return radiusValue / PIXELS_PER_INCH;
                  })(),
                  shadow: shadow
                }
              });
...
async function convertSlide(htmlFile, pres, options = {}) {
  const {
