#!/usr/bin/env python3
"""
Generate thumbnail grids from PowerPoint presentation slides.

Generates a grid layout of slide previews with configurable columns (max 6).
Each grid contains up to cols*(cols+1) images. For presentations with more
slides, multiple numbered grid files are generated automatically.

The program outputs the names of all files generated.
...
Usage:
    python slidePreview.py input.pptx [output_prefix] [--cols N] [--outline-placeholders]

Examples:
    python slidePreview.py presentation.pptx
    # Generates: thumbnails.jpg (using default prefix)
    # Outputs:
    #   Generated 1 grid(s):
    #     - thumbnails.jpg

    python slidePreview.py large-deck.pptx grid --cols 4
    # Generates: grid-1.jpg, grid-2.jpg, grid-3.jpg
    # Outputs:
    #   Generated 3 grid(s):
    #     - grid-1.jpg
    #     - grid-2.jpg
    ...
    python slidePreview.py template.pptx analysis --outline-placeholders
    # Generates thumbnail grids with red outlines around text placeholders
"""
...
from textExtractor import get_text_shapes_inventory as extract_text_inventory
