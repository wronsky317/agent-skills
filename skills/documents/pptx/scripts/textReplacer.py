...
Usage:
    python textReplacer.py <input.pptx> <replacements.json> <output.pptx>

The replacements JSON should have the structure output by textExtractor.py.
ALL text shapes identified by textExtractor.py will have their text cleared
...
from textExtractor import InventoryData, get_text_shapes_inventory
...
def apply_replacements(pptx_file: str, json_file: str, output_file: str):
    ...
    inventory = get_text_shapes_inventory(Path(pptx_file), prs)
    ...
    # (get_text_shapes_inventory accesses font.color which adds empty <a:solidFill/> elements)
    import tempfile
    ...
    try:
        updated_inventory = get_text_shapes_inventory(tmp_path)
