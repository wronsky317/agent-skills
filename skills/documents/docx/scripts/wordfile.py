...
Usage:
    from skills.docx-v2.scripts.wordfile import WordFile

    # Initialize
    doc = WordFile('workspace/unpacked')
    doc = WordFile('workspace/unpacked', author="John Doe", initials="JD")

    # Find nodes
    node = doc["word/document.xml"].locate_element(tag="w:del", attrs={"w:id": "1"})
    node = doc["word/document.xml"].locate_element(tag="w:p", line_number=10)

    # Add comments
    doc.insert_comment(start=node, end=node, text="Comment text")
    doc.respond_to_comment(parent_comment_id=0, text="Reply text")

    # Suggest tracked changes
    doc["word/document.xml"].mark_for_deletion(node)  # Delete content
    doc["word/document.xml"].undo_insertion(ins_node)  # Reject insertion
    doc["word/document.xml"].undo_deletion(del_node)  # Reject deletion
...
from openxml.scripts.assemble import assemble_document
from openxml.scripts.validation.docx import DOCXSchemaValidator
from openxml.scripts.validation.redlining import RedliningValidator

from .xml_helper import XMLProcessor

# Path to template files
TEMPLATE_DIR = Path(__file__).parent / "tpl"


class WordXMLProcessor(XMLProcessor):
    """XMLProcessor that automatically applies RSID, author, and date to new elements.
    ...
    def swap_element(self, elem, new_content):
        """Replace node with automatic attribute injection."""
        nodes = super().swap_element(elem, new_content)
        self._inject_attributes_to_nodes(nodes)
        return nodes

    def add_after(self, elem, xml_content):
        """Insert after with automatic attribute injection."""
        nodes = super().add_after(elem, xml_content)
        self._inject_attributes_to_nodes(nodes)
        return nodes

    def add_before(self, elem, xml_content):
        """Insert before with automatic attribute injection."""
        nodes = super().add_before(elem, xml_content)
        self._inject_attributes_to_nodes(nodes)
        return nodes

    def add_to(self, elem, xml_content):
        """Append to with automatic attribute injection."""
        nodes = super().add_to(elem, xml_content)
        self._inject_attributes_to_nodes(nodes)
        return nodes

    def undo_insertion(self, elem):
        ...
        Example:
            ...
            doc["word/document.xml"].undo_insertion(ins)

            # Reject all insertions in a paragraph
            para = doc["word/document.xml"].locate_element(tag="w:p", line_number=42)
            doc["word/document.xml"].undo_insertion(para)
        """
        # Collect insertions
        ins_elements = []
        ...
        if not ins_elements:
            raise ValueError(
                f"undo_insertion requires w:ins elements. "
    ...
    def undo_deletion(self, elem):
        ...
        Example:
            ...
            nodes = doc["word/document.xml"].undo_deletion(del_elem)

            # Reject all deletions in a paragraph - returns [para]
            para = doc["word/document.xml"].locate_element(tag="w:p", line_number=42)
            nodes = doc["word/document.xml"].undo_deletion(para)
        """
        ...
        if not del_elements:
            raise ValueError(
                f"undo_deletion requires w:del elements. "
        ...
        for del_elem in del_elements:
            ...
            nodes = self.add_after(del_elem, ins_elem.toxml())
    ...
    @staticmethod
    def wrap_paragraph_insertion(xml_content: str) -> str:
    ...
    def mark_for_deletion(self, elem):
...
class WordFile:
    ...
    def __init__(
        ...
        self.temp_dir = tempfile.mkdtemp(prefix="wordfile_")
        ...
        assemble_document(self.original_path, self.original_docx, validate=False)
        ...
        # Convenient access to document.xml processor (semi-private)
    ...
    def __getitem__(self, xml_path: str) -> WordXMLProcessor:
        """
        Get or create a WordXMLProcessor for the specified XML file.

        Enables lazy-loaded processors with bracket notation:
            node = doc["word/document.xml"].locate_element(tag="w:p", line_number=42)

        Args:
            xml_path: Relative path to XML file (e.g., "word/document.xml", "word/comments.xml")

        Returns:
            WordXMLProcessor instance for the specified file
        ...
        Example:
            ...
            comment = doc["word/comments.xml"].locate_element(tag="w:comment", attrs={"w:id": "0"})
        """
        if xml_path not in self._processors:
            file_path = self.unpacked_path / xml_path
            if not file_path.exists():
                raise ValueError(f"XML file not found: {xml_path}")
            # Use WordXMLProcessor with RSID, author, and initials for all processors
            self._processors[xml_path] = WordXMLProcessor(
                file_path, rsid=self.rsid, author=self.author, initials=self.initials
            )
        return self._processors[xml_path]

    def insert_comment(self, start, end, text: str) -> int:
        """
        ...
        Example:
            ...
            cm.insert_comment(start=start_node, end=end_node, text="Explanation")
        """
        ...
        self._document.add_before(start, self._comment_range_start_xml(comment_id))

        # If end node is a paragraph, append comment markup inside it
        # Otherwise insert after it (for run-level anchors)
        if end.tagName == "w:p":
            self._document.add_to(end, self._comment_range_end_xml(comment_id))
        else:
            self._document.add_after(end, self._comment_range_end_xml(comment_id))
    ...
    def respond_to_comment(
        ...
        Example:
            cm.respond_to_comment(parent_comment_id=0, text="I agree with this change")
        """
        ...
        # Add comment ranges to document.xml immediately
        parent_start_elem = self._document.locate_element(
            tag="w:commentRangeStart", attrs={"w:id": str(parent_comment_id)}
        )
        parent_ref_elem = self._document.locate_element(
            tag="w:commentReference", attrs={"w:id": str(parent_comment_id)}
        )

        self._document.add_after(
    ...
    def persist(self, destination=None, validate=True) -> None:
        """
        Save all modified XML files to disk and copy to destination directory.

        This persists all changes made via insert_comment() and respond_to_comment().
    ...
    def _get_next_comment_id(self):
        ...
        processor = self["word/comments.xml"]
        max_id = -1
        for comment_elem in processor.dom.getElementsByTagName("w:comment"):
    ...
    def _add_content_type_for_people(self, path):
        ...
        processor = self["[Content_Types].xml"]

        if self._has_override(processor, "/word/people.xml"):
            return

        # Add Override element
        root = processor.dom.documentElement
    ...
    def _update_settings(self, path, track_revisions=False):
        ...
        """
        processor = self["word/settings.xml"]
        root = processor.locate_element(tag="w:settings")
        ...
        if track_revisions:
            track_revisions_exists = any(
                ...
                for elem in processor.dom.getElementsByTagName(f"{prefix}:trackRevisions")
            )
            ...
            if not track_revisions_exists:
                ...
                for tag in [f"{prefix}:documentProtection", f"{prefix}:defaultTabStop"]:
                    elements = processor.dom.getElementsByTagName(tag)
                    if elements:
                        processor.add_before(elements[0], track_rev_xml)
                        inserted = True
                        break
                if not inserted:
                    # Insert as first child of settings
                    if root.firstChild:
                        processor.add_before(root.firstChild, track_rev_xml)
                    else:
                        processor.add_to(root, track_rev_xml)

        # Always check if rsids section exists
        rsids_elements = processor.dom.getElementsByTagName(f"{prefix}:rsids")
  ...
  <{prefix}:rsid {prefix}:val="{self.rsid}"/>
            ...
            inserted = False
            compat_elements = processor.dom.getElementsByTagName(f"{prefix}:compat")
            if compat_elements:
                processor.add_after(compat_elements[0], rsids_xml)
                inserted = True

            if not inserted:
                clr_elements = processor.dom.getElementsByTagName(
                    f"{prefix}:clrSchemeMapping"
                )
                if clr_elements:
                    processor.add_before(clr_elements[0], rsids_xml)
                    inserted = True

            if not inserted:
                processor.add_to(root, rsids_xml)
        else:
            ...
            if not rsid_exists:
                ...
                processor.add_to(rsids_elem, rsid_xml)
    ...
    def _add_to_comments_xml(
        ...
        processor = self["word/comments.xml"]
        root = processor.locate_element(tag="w:comments")

        escaped_text = (
            text.replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;")
        )
        # Note: w:rsidR, w:rsidRDefault, w:rsidP on w:p, w:rsidR on w:r,
        # and w:author, w:date, w:initials on w:comment are automatically added by WordXMLProcessor
  ...
  <w:p w14:paraId="{para_id}" w14:textId="77777777">
    ...
    <w:r><w:rPr><w:color w:val="000000"/><w:sz w:val="20"/><w:szCs w:val="20"/></w:rPr><w:t>{escaped_text}</w:t></w:r>
        ...
        processor.add_to(root, comment_xml)
    ...
    def _add_to_comments_extended_xml(self, para_id, parent_para_id):
        ...
        processor = self["word/commentsExtended.xml"]
        root = processor.locate_element(tag="w15:commentsEx")
    ...
    def _add_to_comments_ids_xml(self, para_id, durable_id):
        ...
        processor = self["word/commentsIds.xml"]
        root = processor.locate_element(tag="w16cid:commentsIds")
    ...
    def _add_to_comments_extensible_xml(self, durable_id):
        ...
        processor = self["word/commentsExtensible.xml"]
        root = processor.locate_element(tag="w16cex:commentsExtensible")
    ...
    def _comment_range_end_xml(self, comment_id):
        ...
        Note: w:rsidR is automatically added by WordXMLProcessor.
        """
...
<w:r>
  ...
  <w:commentReference w:id="{comment_id}"/>
    ...
    def _has_relationship(self, processor, target):
        """Check if a relationship with given target exists."""
        for rel_elem in processor.dom.getElementsByTagName("Relationship"):
            if rel_elem.getAttribute("Target") == target:
                return True
        return False

    def _has_override(self, processor, part_name):
        """Check if an override with given part name exists."""
        for override_elem in processor.dom.getElementsByTagName("Override"):
            if override_elem.getAttribute("PartName") == part_name:
                return True
        return False

    def _has_author(self, processor, author):
        """Check if an author already exists in people.xml."""
        for person_elem in processor.dom.getElementsByTagName("w15:person"):
    ...
    def _add_author_to_people(self, author):
        ...
        processor = self["word/people.xml"]
        root = processor.locate_element(tag="w15:people")

        # Check if author already exists
        if self._has_author(processor, author):
            return
  ...
  <w15:presenceInfo w15:providerId="None" w15:userId="{escaped_author}"/>
        ...
        processor.add_to(root, person_xml)

    def _ensure_comment_relationships(self):
        """Ensure word/_rels/document.xml.rels has comment relationships."""
        processor = self["word/_rels/document.xml.rels"]

        if self._has_relationship(processor, "comments.xml"):
            return

        root = processor.dom.documentElement
        root_tag = root.tagName  # type: ignore
        prefix = root_tag.split(":")[0] + ":" if ":" in root_tag else ""
        next_rid_num = int(processor.get_next_relationship_id()[3:])
    ...
    def _ensure_comment_content_types(self):
        ...
        if self._has_override(processor, "/word/comments.xml"):
            return
