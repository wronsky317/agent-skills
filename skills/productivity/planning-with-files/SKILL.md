...
- Record verification evidence or explicitly mark verification as pending
...
Before declaring a planned task complete:

- [ ] `task_plan.md` or the active isolated plan has every phase marked complete or intentionally deferred.
- [ ] `progress.md` records the major actions taken and files changed.
- [ ] `findings.md` contains durable discoveries that affected decisions.
- [ ] Verification commands, outputs, screenshots, rendered artifacts, or blockers are recorded.
- [ ] Any skipped checks are explicit in the final report.
